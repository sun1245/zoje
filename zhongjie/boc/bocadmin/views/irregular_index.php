<div class="btn-group">
	<a href="<?php echo site_urlc('irregular/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
<script>
	require(['adminer/js/ui'],function(ui){
		var irregular = {
			url_del: "<?php echo site_urlc('irregular/delete'); ?>"
			,url_audit: "<?php echo site_urlc('irregular/audit'); ?>"
			,url_flag: "<?php echo site_urlc('irregular/flag'); ?>"
			,url_sortid: "<?php echo site_urlc('irregular/sortid'); ?>"
			,url_sort_change: "<?php echo site_urlc('irregular/sort_change'); ?>"
			,url_copy: "<?php echo site_urlc('irregular/copypro'); ?>"
		};
		ui.fancybox_img();
	ui.btn_delete(irregular.url_del);		// 删除
	ui.btn_audit(irregular.url_audit);	// 审核
	ui.btn_flag(irregular.url_flag);		// 推荐
	ui.sortable(irregular.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(irregular.url_sort_change); // input 排序
	ui.btn_copy(irregular.url_copy);    // 复制
});
</script>
